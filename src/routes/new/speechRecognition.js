...
function init() {
  ...
  const onResults = event => {
    ...
    if (resultFinal) {
      ...
      recognition.stop(started_value && !paused_value);
    }
  }
  ...
  if (browser) {
    ...
    // micLevel and audioRecorder have navigator.mediaDevices.getUserMedia() to prompt user for mic permission
    // speechRecognition only prompt user when it starts.
  }
...
export const speechRecognition = readable({
  start: () => {
    recognition.start();
  },
  pause: () => {
